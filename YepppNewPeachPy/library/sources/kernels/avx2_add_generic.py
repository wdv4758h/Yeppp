...
def add_generic(arg_x, arg_y, arg_z, arg_n):
    ...
    JZ(ret_null_pointer)
    TEST(reg_z_addr, arg_z.c_type.base.size - 1) # Make sure arg_z is aligned on the proper boundary
    ...
    with align_loop:
        ...
        scalar_add_instr_select(reg_x_scalar, reg_y_scalar, arg_z.c_type.base)
    ...
    for i in range(unroll_factor):
        with instruction_columns[0]:
            packed_mov_instr_select(ymm_accs[i], [reg_x_addr + i * YMMRegister.size * arg_x.c_type.base.size / arg_z.c_type.base.size], arg_x.c_type.base, arg_z.c_type.base)
        with instruction_columns[1]:
            packed_mov_instr_select(ymm_ops[i], [reg_y_addr + i * YMMRegister.size * arg_x.c_type.base.size / arg_z.c_type.base.size], arg_x.c_type.base, arg_z.c_type.base)
        with instruction_columns[2]:
            packed_add_map[arg_z.c_type.base](ymm_accs[i], ymm_accs[i], ymm_ops[i])
        with instruction_columns[3]:
            packed_aligned_move_map[arg_z.c_type.base]([reg_z_addr + i * YMMRegister.size], ymm_accs[i])
    with instruction_columns[0]:
        ADD(reg_x_addr, YMMRegister.size * unroll_factor * arg_x.c_type.base.size / arg_z.c_type.base.size)
    with instruction_columns[1]:
        ADD(reg_y_addr, YMMRegister.size * unroll_factor * arg_x.c_type.base.size / arg_z.c_type.base.size)
    with instruction_columns[3]:
        ADD(reg_z_addr, YMMRegister.size * unroll_factor)

    software_pipelined_loop(reg_length, unroll_factor * YMMRegister.size / arg_z.c_type.base.size, instruction_columns, instruction_offsets)
