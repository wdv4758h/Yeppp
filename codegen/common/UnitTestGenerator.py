...
class UnitTestGenerator:
    ...
    def __init__(self, name, arguments):
        ...
        # Additional locals:
        # TODO need to set these at some point
        self.ref_array = None
    ...
    def _argument_loop_body(self, utg):
        utg.add_line("memcpy({0}, {1}, sizeof({0}))".format(self.ref_array, self.init_array))
        ...
        utg.add_line("status = defaultImplementation({}, {}, length);".format(
            ", ".join([ "&" + i.name + "Array[" + i.name + "Offset]" for i in inputs ]),
            "&" + output.name + "RefArray[" + output.name + "Offset]"))
        utg.add_line("assert(status == YepStatusOk);")
        utg.add_line()

        utg.add_line("memcpy({0}Array, {0}InitArray, sizeof({0}Array));".format(output.name))
        utg.add_line("status = descriptor->function({}, {}, length);".format(
            ", ".join([ "&" + i.name + "Array[" + i.name + "Offset]" for i in inputs ]),
            "&" + output.name + "Array[" + i.name + "Offset]"))
        utg.add_line("if (memcmp({0}Array, {0}RefArray, sizeof({0}Array)) != 0) {{".format(output.name)).indent()
        ...
        utg.dedent().add_line("}").dedent().add_line("}").dedent()
