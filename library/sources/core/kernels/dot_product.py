...
def dot_product_Haswell(arg_x, arg_y, arg_z, arg_n):
    op_size = arg_x.c_type.base.size # 4 byte operands
    ...
    JZ(align_loop.end)
    with align_loop:
        SCALAR_MOV(op_reg_out, [reg_x_addr])
        SCALAR_MOV(op_reg, [reg_y_addr])
        SCALAR_MUL(op_reg_out, op_reg_out, op_reg)
        SCALAR_ADD(scalar_acc.as_xmm, scalar_acc.as_xmm, op_reg_out)
    ...
    for i in range(unroll_factor):
        with instruction_columns[0]:
            SIMD_ALIGNED_MOV(op_regs_out[i], [reg_x_addr + i * YMMRegister.size])
        with instruction_columns[1]:
            SIMD_MOV(op_regs[i], [reg_y_addr + i * YMMRegister.size])
        with instruction_columns[2]:
            SIMD_MUL(op_regs_out[i], op_regs_out[i], op_regs[i])
        with instruction_columns[3]:
            SIMD_ADD(acc_regs[i], acc_regs[i], op_regs_out[i])
    with instruction_columns[0]:
