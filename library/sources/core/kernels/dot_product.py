...
def dot_product_V64fV64f_S64f_Haswell(arg_x, arg_y, arg_z, arg_n):
    op_size = 8 # 4 byte operands
    unroll_factor = 5
    ...
    # Zero out all of the accumulation registers
    for acc_reg in acc_regs:
        VXORPD(acc_reg, acc_reg, acc_reg)
    VXORPD(scalar_acc, scalar_acc, scalar_acc)
    ...
    JZ(align_loop.end)
    with align_loop:
        VMOVSD(op_reg_out, [reg_x_addr])
        VMOVSD(op_reg, [reg_y_addr])
        VMULSD(op_reg_out, op_reg_out, op_reg)
        VADDSD(scalar_acc.as_xmm, scalar_acc.as_xmm, op_reg_out)
        ADD(reg_x_addr, op_size)
        ADD(reg_y_addr, op_size)
        SUB(reg_length, 1)
        JZ(ret_ok)
    ...
    for i in range(unroll_factor):
        with instruction_columns[0]:
            VMOVAPD(op_regs_out[i], [reg_x_addr + i * YMMRegister.size])
        with instruction_columns[1]:
            VMOVUPD(op_regs[i], [reg_y_addr + i * YMMRegister.size])
        with instruction_columns[2]:
            VMULPD(op_regs_out[i], op_regs_out[i], op_regs[i])
        with instruction_columns[3]:
            VADDPD(acc_regs[i], acc_regs[i], op_regs_out[i])
    with instruction_columns[0]:
        ADD(reg_x_addr, YMMRegister.size * unroll_factor)
    with instruction_columns[1]:
        ADD(reg_y_addr, YMMRegister.size * unroll_factor)
    ...
    with LABEL(ret_ok):
        ...
        # Reduce the accumulators
        for i in range(1, len(acc_regs)):
            VADDPD(acc_regs[0], acc_regs[0], acc_regs[i])
        VADDPD(acc_regs[0], acc_regs[0], scalar_acc)
        ...
        # Move the upper sum into the lower bits of tmp and then add them to get the sum of all 8 elements in the accumlator
        VPERM2F128(tmp, acc_regs[0], acc_regs[0], 0x1)
        VADDSD(acc_regs[0].as_xmm, acc_regs[0].as_xmm, tmp.as_xmm)

        VMOVSD([reg_z_addr], acc_regs[0].as_xmm)
        RETURN(YepStatus.YepStatusOk)
